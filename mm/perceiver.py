...
class PerceiverAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, latents: torch.Tensor, media_padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q = q.view(q.shape[0], q.shape[1], q.shape[2] // h, h).transpose(1, 2)
        k = k.view(k.shape[0], k.shape[1], k.shape[2] // h, h).transpose(1, 2)
        v = v.view(v.shape[0], v.shape[1], v.shape[2] // h, h).transpose(1, 2)
        q = q * self.scale
        ...
        out = out.transpose(1, 2).contiguous().view(out.shape[0], out.shape[2], h * out.shape[-1])
