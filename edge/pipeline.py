...
def build_exact_r_list(drop_tokens: int, allocator: CNA_Allocator) -> List[int]:
    """Allocate an exact ToMe drop budget across layers using CNA's layer weights.

    CNA_Allocator.generate_r_list is bandwidth-aware and intentionally nonlinear,
    so it does not preserve fixed offline retain ratios. For dataset construction
    we need the sum of r_list to match the requested token drop budget.
    """
...
def build_sparse_source_payload(model, actual_keep_tokens: int) -> Dict[str, object]:
    ...
    # source: [B, N_after_with_cls, N_before_with_cls]. Drop CLS rows and columns.
...
def quantize_features_int8_per_token(features: torch.Tensor) -> Dict[str, object]:
    """Symmetric int8 quantization with one scale per visual token."""
